...
# MAGIC %md-sandbox
# MAGIC # 2/ Creating the Chat bot with Retrieval Augmented Generation (RAG)
# MAGIC
...
# MAGIC
# MAGIC
# MAGIC <!-- Collect usage data (view). Remove it to disable collection or disable tracker during installation. View README for more details.  --><img src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-science&org_id=1444828305810485&notebook=02-Deploy-RAG-Chatbot-Model&demo_name=chatbot-rag-llm&event=VIEW">
...
# MAGIC %pip install --force-reinstall "/Workspace/Wheels/langchain/langchain-0.0.339+databricks.vectorsearch-py3-none-any.whl" databricks-vectorsearch==0.21 mlflow==2.8.0 databricks-sdk==0.12.0 databricks-genai-inference==0.1.1 mlflow[databricks] openai==1.3.2
...
# MAGIC - You'll need to setup the Databricks CLI in your laptop or using this cluster terminal: <br/>
...
# MAGIC                                 changes=[c.PermissionsChange(add=[c.Privilege["READ"]], principal="<YOUR_SP>")])
# MAGIC   ```
...
# MAGIC * Calling the vector search index to find similary documents to augment the prompt with
# MAGIC
...
os.environ['DATABRICKS_TOKEN'] = dbutils.secrets.get("doan-demos", "rag_sp_token")
...
embedding_model = DatabricksEmbeddings(model="bge-large-en", host=host)
...
similar_documents = vectorstore.get_relevant_documents("How do I trach my Databricks Billing?")
...
chat_model = DatabricksChatModel(model="llama-2-70b-chat")
print(f"Test chat model: {chat_model.predict('What is Apache Spark', max_tokens=200)}")

# COMMAND ----------

# MAGIC %md-sandbox
# MAGIC
# MAGIC ### Assembling the compete RAG Chain
# MAGIC
...
# MAGIC
# MAGIC
# MAGIC Let's now merge the retriever and the mode in a single Langchain chain.
# MAGIC
# MAGIC We will use a custom langchain template for our assistant to give proper answer.
# MAGIC
# MAGIC Make sure you take some time to try different templates and adjust your assistant personality for your requirement.
# MAGIC
# MAGIC
# MAGIC
...
import databricks_genai_inference
...
#------------------------
# TODO: this will be replaced with DatabricksChatModel once available in langchain before the final demo release.
# Use the MLFlowAIGateway with an existing AI Gateway for the time being
from langchain.llms import MlflowAIGateway
chat_model = MlflowAIGateway(
    gateway_uri=os.environ["MLFLOW_GATEWAY_URI"],
    route="mosaicml-llama2-70b-completion",
    params={"max_tokens": 500, "stop": []},
    verbose=True
)
#------------------------
...
question = {"query": "How can I leverage GPUs for training a PyTorch classification model?"}
...
with mlflow.start_run(run_name="dbdemos_chatbot_rag") as run:
    ...
    model_info = mlflow.langchain.log_model(
        ...
        pip_requirements=[
            ...
            "/Workspace/Wheels/langchain/langchain-0.0.339+databricks.vectorsearch-py3-none-any.whl",  # TODO: replace this with langchain
            "databricks-vectorsearch",
            "databricks-genai-inference==" + databricks_genai_inference.__version__,
        ],
        input_example=question,
        signature=signature
    )
    #------------------------
    # TODO: temporary fix to add the wheel, we won't need this after we release the VS<>LangChain
...
# MAGIC
# MAGIC We'll then be able to sending requests from our assistnat frontend.
...
serving_endpoint_name = f"dbdemos_chatbot_endpoint_{catalog}_{db}"
...
question = "Can I use the ML DBR with Delta Live Tables and Unity Catalog?"
...
# MAGIC
# MAGIC *Note: this UI is hosted and maintained by Databricks for demo purpose and don't use the model you just created.*
